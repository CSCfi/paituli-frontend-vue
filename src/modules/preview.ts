...
// format names.
export function isPackageEntry(path: string): boolean {
...
export function previewBlocker(
  ...
  if (isPackageEntry(file)) return 'unresolved'
...
export function rendererFor(source: PreviewSource): Component {
  if (source.directory || isPackageEntry(source.file)) return unsupportedPreview
