...
async function load() {
  ...
  try {
    ...
    // Unknown projections, missing files and CORS refusals all land here.
    // geotiff.js reports a failed read as "Error fetching data", so the status
    // the request actually got is worth more than the message.
    console.error('Preview could not render GeoTIFF:', cause)
    const message = cause instanceof Error ? cause.message : String(cause)
    error.value = transfer.failedStatus.value
      ? t('http', { status: transfer.failedStatus.value })
      : message
    loading.value = false
  }
}
...
<i18n>
{
  "en": {
    ...
    "http": "The file could not be fetched (HTTP {status})",
    "band": "Band",
    "of": "of {count}",
  ...
  "fi": {
    ...
    "http": "Tiedostoa ei voitu hakea (HTTP {status})",
    "band": "Kanava",
    "of": "/ {count}",
